...
// `nothing/none/nobody` added 2026-08-03: these are DENIALS of a claim ("nothing shipped", "none of
// it works", "nothing was committed"), and without them a sentence stating the OPPOSITE of done was
// read as done — the gate bounced a turn for reporting that it had shipped nothing.
const NEG = /(?:\bnothing\b|\bnone\b|\bnobody\b|\bnot\b|\bno\s+longer\b|\bnever\b|\bcannot\b|\bcan'?t\b|\bwon'?t\b|\bisn'?t\b|\baren'?t\b|\bwasn'?t\b|\bweren'?t\b|\bdoesn'?t\b|\bdon'?t\b|\bdidn'?t\b|\bhasn'?t\b|\bhaven'?t\b|\bwouldn'?t\b|\bcouldn'?t\b|\bshouldn'?t\b|\bunverified\b|\bunconfirmed\b|\buntested\b|\bfails?\s+to\b|\bfailing\s+to\b|\bunable\s+to\b)\s+(?:yet\s+|quite\s+|fully\s+|currently\s+|really\s+|actually\s+|even\s+|still\s+|consistently\s+|always\s+|properly\s+|completely\s+|reliably\s+|necessarily\s+|is\s+|was\s+|are\s+|were\s+|of\s+it\s+|of\s+this\s+|of\s+them\s+){0,3}$/i;
