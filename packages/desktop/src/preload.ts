...
/**
 * Window-level powers the sandboxed renderer cannot have on its own.
 * Deliberately tiny and kept SEPARATE from the `narukami` bridge below, because
 * the SPA treats the mere presence of `window.narukami` as "I'm in the desktop
 * shell with tear-off support" — the PC Stats utility window has no dock, so it
 * must not answer to that check.
 */
